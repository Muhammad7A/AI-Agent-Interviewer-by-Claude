...
def run_study(
    *, llm: LLMClient | None = None, candor: str = "open", max_turns: int = 14
) -> StudyResult:
    personas: list[Persona] = meridian_org(candor)
    ...
    for topic in aggregation.findings:
        ...
        for i, (member, score) in enumerate(zip(topic.members, scores)):
            ...
            correct = matched is not None and is_veridical(matched.id)
...
def render_study_report(study: StudyResult, *, mode: str) -> str:
    ...
    lines.append("|---|---|---|---|---|---|---|")
    for r in sorted(study.rows, key=lambda r: r.confidence.value, reverse=True):
        lines.append(
